...
class SignalGenerationTester:
    ...
    async def _check_server_status(self):
        ...
        try:
            ...
            print("   [INFO] Server is not running, starting it...")
    ...
    async def _validate_response_format(self):
        ...
        if not self.test_results["api_response"]:
            print("   [ERROR] No API response to validate")
            return
        ...
        if "final_decision" in response:
            ...
            for field in decision_fields:
                if field in final_decision:
                    print(f"   [OK] Found final_decision field: {field}")
                    validation_results[f"has_final_decision_{field}"] = True
                else:
                    print(f"   [ERROR] Missing final_decision field: {field}")
            ...
            if "signal" in final_decision:
                ...
                if signal in ["BUY", "SELL", "HOLD"]:
                    print(f"   [OK] Valid signal value: {signal}")
                    validation_results["valid_signal_value"] = True
                else:
                    print(f"   [ERROR] Invalid signal value: {signal}")
            ...
            if "confidence" in final_decision:
                ...
                if isinstance(confidence, (int, float)) and 0.0 <= confidence <= 1.0:
                    print(f"   [OK] Valid confidence value: {confidence}")
                    validation_results["valid_confidence_value"] = True
                else:
                    print(f"   [ERROR] Invalid confidence value: {confidence}")
        ...
        if "agent_decisions" in response:
            ...
            for agent in expected_agents:
                if agent in agent_decisions:
                    print(f"   [OK] Found agent decision: {agent}")
        ...
        if "agent_decisions" in response and "technical" in response["agent_decisions"]:
            ...
            if "supporting_data" in technical_decision:
                ...
                if "signal_source" in supporting_data:
                    ...
                    print(f"   [OK] Signal source detected: {signal_source}")

                    if signal_source == "LOCAL":
                        print("   [OK] Local Signal Generation Framework is being used!")
                        local_generation_detected = True
                    elif signal_source == "LLM":
                        print("   [WARNING] LLM-based signal generation (Local framework not used)")
                        ...
                        print("   [WARNING] Hybrid mode: Local signal escalated to LLM")
                        local_generation_detected = True
                        self.test_results["recommendations"].append(
                ...
                if "local_generation_metadata" in supporting_data:
                    print("   [OK] Local generation metadata found")
                    local_generation_detected = True
                    ...
                    if "performance_metrics" in metadata:
                        print("   [OK] Performance metrics found in local generation")
                    ...
                    if "market_regime" in metadata:
                        ...
                        print(f"   [OK] Market regime detected: {regime.get('regime', 'Unknown')}")
                    ...
                    if "escalation" in metadata:
                        ...
                        if escalation.get("required", False):
                            print(f"   [WARNING] Escalation required: {escalation.get('reasoning', 'No reason')}")
                            self.test_results["recommendations"].append(
                ...
                if "signal_strength" in supporting_data:
                    print(f"   [OK] Signal strength: {supporting_data['signal_strength']}")

                if "market_regime" in supporting_data:
                    print(f"   [OK] Market regime: {supporting_data['market_regime']}")

                # Check for indicators
                if "indicators" in supporting_data:
                    indicators = supporting_data["indicators"]
                    print(f"   [OK] Technical indicators calculated: {list(indicators.keys())}")

        if not local_generation_detected:
            print("   [WARNING] Local Signal Generation Framework not detected")
            self.test_results["recommendations"].append(
    ...
    async def _validate_performance_metrics(self):
                ...
                for agent_name, agent_decision in response["agent_decisions"].items():
                    if "supporting_data" in agent_decision:
                        ...
                        if "local_generation_metadata" in supporting_data:
                            metadata = supporting_data["local_generation_metadata"]
                            if "performance_metrics" in metadata:
                                print(f"   [OK] Performance metrics found in {agent_name} agent")
        ...
        if not metrics_found:
            try:
                print("   [INFO] Checking monitoring endpoint for metrics...")
                response = requests.get(f"{self.base_url}/api/v1/monitoring/metrics", timeout=10)

                if response.status_code == 200:
                    metrics = response.json()
                    print("   [OK] Performance metrics found in monitoring endpoint")
                    self._analyze_performance_metrics(metrics)
                    metrics_found = True
                else:
                    print(f"   [WARNING] Monitoring endpoint returned status {response.status_code}")

            except Exception as e:
                print(f"   [WARNING] Could not fetch metrics from monitoring endpoint: {e}")
    ...
    def _analyze_performance_metrics(self, metrics: Dict[str, Any]):
        ...
        if "avg_generation_time" in metrics:
            ...
            if avg_time <= target_time:
                print(f"     [OK] Meets performance target (< {target_time}s)")
            else:
                print(f"     [WARNING] Exceeds performance target (> {target_time}s)")
        ...
        if "local_signals" in metrics and "llm_signals" in metrics:
            ...
            if total > 0:
                ...
                if local_percentage > 50:
                    print("     [OK] Local signal generation is primary")
                else:
                    print("     [WARNING] LLM signal generation is primary")
        ...
        if "escalations" in metrics:
            ...
            if "total_signals_generated" in metrics and metrics["total_signals_generated"] > 0:
                ...
                if escalation_rate > 20:
                    print("     [WARNING] High escalation rate detected")
                    self.test_results["recommendations"].append(
        ...
        if "local_errors" in metrics and "local_signals" in metrics:
            if metrics["local_signals"] > 0:
                ...
                if error_rate > 5:
                    print("     [WARNING] High local signal error rate")
                    self.test_results["issues_found"].append(
    ...
    def _generate_final_report(self):
        ...
        # API Response
        if self.test_results["api_response"]:
            print(f"\nAPI Response: [OK] Successful")
            if "response_time" in self.test_results:
                print(f"Response Time: {self.test_results['response_time']:.2f}s")
        else:
            print(f"\nAPI Response: [ERROR] Failed")

        # Signal Generation
        print(f"\nLocal Signal Generation: {'[OK] Detected' if self.test_results.get('local_generation_detected') else '[ERROR] Not Detected'}")
        ...
        if recommendations:
            ...
            print("\nRecommendations: None [OK]")

        # Overall Status
        print(f"\nOverall Status: {'[OK] PASS' if not issues else '[WARNING] ISSUES DETECTED'}")
